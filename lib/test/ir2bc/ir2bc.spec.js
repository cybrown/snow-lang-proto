...
describe('IR to Bytecode translator', function () {
    ...
    it('IntegerConstant', function () {
        ...
        assert.equal(4 /* CONST */, buffer.readUInt8(0));
    ...
    it('Add', function () {
        ...
        assert.equal(4 /* CONST */, buffer.readUInt8(0));
        assert.equal(1, buffer.readInt32BE(1));
        assert.equal(4 /* CONST */, buffer.readUInt8(5));
        assert.equal(2, buffer.readInt32BE(6));
        assert.equal(2 /* ADD */, buffer.readUInt8(10));
    });
    it('Sub', function () {
        ...
        assert.equal(4 /* CONST */, buffer.readUInt8(5));
        assert.equal(2, buffer.readInt32BE(6));
        assert.equal(3 /* SUB */, buffer.readUInt8(10));
    });
    it('Call', function () {
        ...
        assert.equal(7 /* CALL */, buffer.readUInt8(9));
    });
    it('Func', function () {
        var irAdd = new ir.Add(new ir.IntegerConstant(1), new ir.IntegerConstant(2));
        ...
        assert.equal(2, buffer.readInt32BE(6));
        assert.equal(2 /* ADD */, buffer.readUInt8(10));
        assert.equal(buffer.readUInt8(11), 4 /* CONST */);
        assert.equal(buffer.readInt32BE(12), 1);
        assert.equal(buffer.readUInt8(16), 5 /* RET */);
    });
    it('Return', function () {
        var irRet = new ir.Return(new ir.IntegerConstant(1));
        irTranslator.translateReturn(irRet);
        var buffer = irTranslator.toBuffer();
        assert.equal(buffer.readUInt8(0), 4 /* CONST */);
        assert.equal(buffer.readInt32BE(1), 1);
        assert.equal(buffer.readUInt8(5), 5 /* RET */);
    });
